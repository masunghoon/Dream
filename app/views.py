...
def getUserDday():
    ...
    decade = []
    for i in range(1,10):
        data = {'range': str((i-1)*10) + '\'s', 'dueDate':datetime.strftime(birth+timedelta(3652.5*i),'%Y/%m/%d')}
    ...
    year = []
    for i in range(100):
        data = {'range':'Year '+str(birth.year + i), 'dueDate':datetime.strftime(date(birth.year+i,12,31),'%Y/%m/%d')}
...
class BucketAPI(Resource):
    ...
    def get(self, id):
        ...
        t = Bucket.query.filter_by(parentID = id).all()
        tododata = []
        for i in t:
            tododata.append({
                        'id':i.id,
    ...
    def post(self, id):
        if not request.json or not 'title' in request.json:
            abort(400)
        if not 'deadline' in request.json == None:
